...
// FOPS FWDDECL //
static dma_cookie_t dma_prep_buffer(struct zynq_device_dev *dev, struct dma_chan *chan, dma_addr_t buf, size_t len, 
...
int writeBuf(struct zynq_device_dev *dev, u32 sample)
{
...
int writeBufSet(struct zynq_device_dev *dev, u32 *buf, int nSamples)
{
    int i;
    spin_lock_irq(&dev->spinLock);
...
u32 readBuf(struct zynq_device_dev *dev)
{
    u32 data;
    spin_lock_irq(&dev->spinLock);
    if(dev->bufCount <= 0)
...
static void dma_buffer2fifo_buffer(struct zynq_device_dev *dev)
{
    int i;
 ...
 * thread of control
 */
static int dma_sync_callback(struct zynq_device_dev *dev)
{
    int status, i;
...
{    
    if(!file->private_data) {
        ...
        struct zynq_device_dev *privateInfo = container_of(inode->i_cdev, struct zynq_device_dev, cdev);
    ...
    struct zynq_device_dev *privateInfo = (struct zynq_device_dev *)file->private_data;
...
{
    struct zynq_device_dev *dev = file->private_data;
...
{    
    u32 i = 0;
    struct zynq_device_dev *dev = (struct zynq_device_dev *)filp->private_data;
    u32 *b32 = (u32*)buffer;
...
//Start DMA if DMA buffer previusly allocated
	...
	return 0;
      ...
      case ZYNQ_DEVICE_STOP_DMA:
      {
	  dev->dma_started = 0;
	  return 0;
      }
      case ZYNQ_DEVICE_GET_DMA_BUFLEN:
      {
	  copy_to_user ((void __user *)arg, &dev->dma_buf_size, sizeof(u32));	
	  return 0;
      }
      case ZYNQ_DEVICE_SET_DRIVER_BUFLEN:
      {
	  if(dev->dma_started)
	      return 0;
	  dev->wIdx = 0;
	...
	case ZYNQ_DEVICE_SET_CONFIGURATION:
	{
		struct zynq_device_conf currConf;
...
{
    unsigned int mask=0;
    struct zynq_device_dev *dev =  (struct zynq_device_dev *)file->private_data;
...
static int id_major;
static struct class *zynq_device_class;
static struct zynq_device_dev staticPrivateInfo;
static int zynq_device_dma_probe(struct platform_device *pdev)
{
    int i;
    u32 off;
    static int memIdx;
    ...
    //s_pdev = pdev;
    printk("zynq_device_dma_probe  %x\n", pdev->name);
	...
	memIdx = 0;
        ...
        zynq_device_class = class_create(THIS_MODULE, DEVICE_NAME);
        printk("CHIAMATO CLASS CREATE %d\n",  zynq_device_class);
        if (IS_ERR(zynq_device_class))
            return PTR_ERR(zynq_device_class);

        printk("DEVICE CREATE...\n");
        device_create(zynq_device_class, NULL, MKDEV(id_major, 0),
                  NULL, DEVICE_NAME);
    ...
    {
      printk("SUCCESSIVA CHIAMATA A rfx_zynq_device_dma_probe: %s, memIdx: %d\n", pdev->name, memIdx);	
...
*/	default: printk("ERROR: Unexcpected zynq_device_dma_probe call\n");
      }
      memIdx++;
...
static int zynq_device_dma_remove(struct platform_device *pdev)
{
    printk("PLATFORM DEVICE REMOVE...\n");
    if(zynq_device_class) {
        device_destroy(zynq_device_class,MKDEV(id_major, 0));
        class_destroy(zynq_device_class);
    }
...
static const struct of_device_id zynq_device_dma_of_ids[] = {
{ .compatible = "xlnx,axi-dma-test-1.00.a",},
{ .compatible = "xlnx,axi-cfg-register-1.0",},
{}
};

static struct platform_driver zynq_device_dma_driver = {
   .driver = {
        .name  = MODULE_NAME,
        .owner = THIS_MODULE,
        .of_match_table = zynq_device_dma_of_ids,
     },
    .probe = zynq_device_dma_probe,
    .remove = zynq_device_dma_remove,
};

static int __init zynq_device_dma_init(void)
{
    printk(KERN_INFO "inizializing AXI module ...\n");
    deviceAllocated = 0;
    return platform_driver_register(&zynq_device_dma_driver);
}

static void __exit zynq_device_dma_exit(void)
{
    printk(KERN_INFO "exiting AXI module ...\n");
    platform_driver_unregister(&zynq_device_dma_driver);
}

module_init(zynq_device_dma_init);
module_exit(zynq_device_dma_exit);
MODULE_LICENSE("GPL");
