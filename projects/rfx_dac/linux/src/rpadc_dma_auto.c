...
#ifdef HAS_DMA
static dma_cookie_t dma_prep_buffer(struct rpadc_dma_auto_dev *dev, struct dma_chan *chan, dma_addr_t buf, size_t len, 
...
int writeBuf(struct rpadc_dma_auto_dev *dev, u32 sample)
{
...
int writeBufSet(struct rpadc_dma_auto_dev *dev, u32 *buf, int nSamples)
{
    int i;
    spin_lock_irq(&dev->spinLock);
...
u32 readBuf(struct rpadc_dma_auto_dev *dev)
{
    u32 data;
    spin_lock_irq(&dev->spinLock);
    if(dev->bufCount <= 0)
...
#ifdef HAS_DMA
static void dma_buffer2fifo_buffer(struct rpadc_dma_auto_dev *dev)
{
    int i;
 ...
 * thread of control
 */
static int dma_sync_callback(struct rpadc_dma_auto_dev *dev)
{
    int status, i;
...
{    
    if(!file->private_data) {
        ...
        struct rpadc_dma_auto_dev *privateInfo = container_of(inode->i_cdev, struct rpadc_dma_auto_dev, cdev);
    ...
    struct rpadc_dma_auto_dev *privateInfo = (struct rpadc_dma_auto_dev *)file->private_data;
...
{
    struct rpadc_dma_auto_dev *dev = file->private_data;
...
{    
    u32 i = 0;
    struct rpadc_dma_auto_dev *dev = (struct rpadc_dma_auto_dev *)filp->private_data;
    u32 *b32 = (u32*)buffer;
...
//	  dmaengine_terminate_all(dev->dma_chan);
      ...
      case RPADC_DMA_AUTO_SET_DRIVER_BUFLEN:
      {
	  if(dev->dma_started)
	      return 0;
	  dev->wIdx = 0;
	...
	case RPADC_DMA_AUTO_SET_REGISTERS:
	{
		struct rpadc_dma_auto_registers currConf;
...
{
    unsigned int mask=0;
    struct rpadc_dma_auto_dev *dev =  (struct rpadc_dma_auto_dev *)file->private_data;
...
static int id_major;
static struct class *rpadc_dma_auto_class;
static struct rpadc_dma_auto_dev staticPrivateInfo;
static int rpadc_dma_auto_probe(struct platform_device *pdev)
{
    int i;
    u32 off;
    static int memIdx;
	...
	memIdx = 0;
        ...
        rpadc_dma_auto_class = class_create(THIS_MODULE, DEVICE_NAME);
        if (IS_ERR(rpadc_dma_auto_class))
            return PTR_ERR(rpadc_dma_auto_class);

        device_create(rpadc_dma_auto_class, NULL, MKDEV(id_major, 0),
                  NULL, DEVICE_NAME);
    ...
    {
//      printk("SUCCESSIVA CHIAMATA A rfx_rpadc_dma_auto_probe: %s, memIdx: %d\n", pdev->name, memIdx);	
...
off = r_mem->start & ~PAGE_MASK;
	...
	case 6:
		...
		off = r_mem->start & ~PAGE_MASK;
		staticPrivateInfo.iomap1_time_fifo = devm_ioremap(&pdev->dev,r_mem->start+off,0xffff); break;


	 default: printk("ERROR: Unexpected rpadc_dma_auto_probe call\n");
      }
      memIdx++;
...
static int rpadc_dma_auto_remove(struct platform_device *pdev)
{
    printk("PLATFORM DEVICE REMOVE...\n");
    if(rpadc_dma_auto_class) {
        device_destroy(rpadc_dma_auto_class,MKDEV(id_major, 0));
        class_destroy(rpadc_dma_auto_class);
    }
...
static const struct of_device_id rpadc_dma_auto_of_ids[] = {
...
static struct platform_driver rpadc_dma_auto_driver = {
   .driver = {
        .name  = MODULE_NAME,
        .owner = THIS_MODULE,
        .of_match_table = rpadc_dma_auto_of_ids,
     },
    .probe = rpadc_dma_auto_probe,
    .remove = rpadc_dma_auto_remove,
};

static int __init rpadc_dma_auto_init(void)
{
    printk(KERN_INFO "inizializing AXI module ...\n");
    deviceAllocated = 0;
    return platform_driver_register(&rpadc_dma_auto_driver);
}

static void __exit rpadc_dma_auto_exit(void)
{
    printk(KERN_INFO "exiting AXI module ...\n");
    platform_driver_unregister(&rpadc_dma_auto_driver);
}

module_init(rpadc_dma_auto_init);
module_exit(rpadc_dma_auto_exit);
MODULE_LICENSE("GPL");
